        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.monitor_mode != 'off' : # Then there is a metric to monitor
                if self.monitor_metric in log: # Then we have measured it in this epoch
                    current_metric_value = log[self.monitor_metric]
                
                    if self.monitor_mode == 'min':
                        improved = current_metric_value < self.monitor_best
                    else:  # self.monitor_mode == 'max'
                        improved = current_metric_value > self.monitor_best

                    if improved:
                        self.monitor_best = current_metric_value
                        self.best_epoch = self.current_epoch
                        self.save_model(path=ospj(self.checkpoint_dir, 'best_model.pth'))
                        self.logger.info(f"Improved {self.monitor_metric} at epoch {self.current_epoch}: {current_metric_value}")
                        self.not_improved_count = 0
                    else:
                        self.not_improved_count += 1

                    if self.not_improved_count > self.early_stop:
                        self.logger.info(f"Early stopping at epoch {self.current_epoch}. No improvement in {self.monitor_metric} for {self.not_improved_count} consecutive epochs.")
                        break
